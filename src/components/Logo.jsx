...
export function LogoMark({ size = 44, radius = 12 }) {
  return (
    <div style={{ width: size, height: size, borderRadius: radius, background: 'var(--surface)', border: '1px solid var(--accent-line)', display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none', boxShadow: '0 0 22px var(--accent-soft) inset' }}>
      <img src={SRC} alt="Khawar Construction" style={{ width: Math.round(size * 0.64), height: 'auto', display: 'block', filter: 'drop-shadow(0 0 6px var(--accent-glow))' }} />
    </div>
  )
}
...
export function LogoImage({ width = 340, float = true }) {
  return <img src={SRC} alt="Khawar Construction" style={{ width, height: 'auto', display: 'block', filter: 'drop-shadow(0 0 30px var(--accent-glow))', animation: float ? 'floatY 6s ease-in-out infinite' : 'none' }} />
}
