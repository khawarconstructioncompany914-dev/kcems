  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
      <div style={{ flex: 1, minWidth: 200 }}>
        ...
        <h1 style={{ font: '700 26px/1.05 var(--f-display)', color: 'var(--text)', letterSpacing: '-.02em', margin: 0 }}>{title}</h1>
...
export function Kpi({ label, value, sub, color = 'var(--text)', accent }) {
  return (
    <div className="card" style={{ padding: '18px 20px', flex: 1, minWidth: 150, position: 'relative', overflow: 'hidden' }}>
      {accent && <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(120% 80% at 100% 0%, var(--accent-soft), transparent 60%)', pointerEvents: 'none' }} />}
      ...
      {sub && <div style={{ font: '500 12px/1.4 var(--f-body)', color: 'var(--text-42)', marginTop: 8 }}>{sub}</div>}
    </div>
  )
}
...
export function SectionTitle({ children, right }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '2px 0 14px' }}>
      <div style={{ font: '700 15px/1 var(--f-body)', color: 'var(--text)' }}>{children}</div>
