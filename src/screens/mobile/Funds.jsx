  ...
  return (
    <div className="field-screen">
      <div style={{ padding: '6px 20px 4px' }}>
        <div style={{ font: '700 20px/1 var(--f-display)', color: 'var(--text)' }}>Funds</div>
      </div>
      ...
      <div style={{ margin: '12px 16px 0', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 18, padding: 18 }}>
        ...
        <div style={{ display: 'flex', gap: 18, marginTop: 12 }}>
          <div><div style={{ font: '600 9px/1 var(--f-mono)', color: 'var(--text-40)' }}>FUNDED</div><div className="num" style={{ font: '700 15px/1 var(--f-display)', color: 'var(--text)', marginTop: 6 }}>{formatMoney(bal.funded)}</div></div>
          <div><div style={{ font: '600 9px/1 var(--f-mono)', color: 'var(--text-40)' }}>SPENT</div><div className="num" style={{ font: '700 15px/1 var(--f-display)', color: 'var(--text)', marginTop: 6 }}>{formatMoney(bal.spent)}</div></div>
      ...
      <div style={{ padding: '18px 16px 0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
          {txns.map((t) => (
            <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 12px', borderRadius: 13, background: 'var(--surface)', border: '1px solid var(--border-3)' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ font: '600 12px/1.2 var(--f-body)', color: 'var(--text)' }}>{t.type === 'settlement' ? 'Settlement' : (t.note || 'Funds added')}</div>
